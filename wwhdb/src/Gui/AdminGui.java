...
public class AdminGui extends JFrame {
	...
	private JTextField fieldAdminRemove;
	...
	public AdminGui(Admin admin) throws SQLException {
		...
		adminListe.setBounds(10, 40, 660, 574);
		panel_1.add(adminListe);
		...
		adminTable.getSelectionModel().addListSelectionListener( new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				try {
					fieldAdminRemove.setText(adminTable.getValueAt(adminTable.getSelectedRow(), 1).toString());
				} catch (Exception ex) {
		...
		fieldAdminRemove = new JTextField();
		fieldAdminRemove.setBounds(10, 54, 255, 20);
		panel_8.add(fieldAdminRemove);
		fieldAdminRemove.setColumns(10);
		...
		fieldPasswortRemove.setBounds(10, 103, 255, 20);
		panel_8.add(fieldPasswortRemove);
		...
		adminDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(Helper.confirm("delete_admin")) {
					if(fieldAdminRemove.getText().length() != 8) {
						...
						boolean control = admin.deleteAdmin(fieldAdminRemove.getText());
						if(control) {
							JOptionPane.showMessageDialog(null,"Admin wird erfolgreich aus der Datenbank gelöscht.");
							fieldAdminRemove.setText(null);
		...
		adminDelete.setBounds(95, 135, 85, 20);
		panel_8.add(adminDelete);
	...
	public static void sucheAdminModel(String column, String key) throws SQLException {
		...
		clearModel.setRowCount(0);
			...
			adminData[0] = admin.adminSuche(column, key).get(0).getId();
			adminData[1] = admin.adminSuche(column, key).get(0).getPersonalId();
			adminData[2] = admin.adminSuche(column, key).get(0).getName();
			adminData[3] = admin.adminSuche(column, key).get(0).getNachname();
			adminData[4] = admin.adminSuche(column, key).get(0).getFiliale();
			adminModel.addRow(adminData);
