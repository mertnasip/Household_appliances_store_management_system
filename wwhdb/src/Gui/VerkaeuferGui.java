...
public class VerkaeuferGui extends JFrame {
	
	static Verkaeufer verkaeufer = new Verkaeufer();
	...
	private JTextField textFieldKundeDelete;
	...
	/*public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					VerkaeuferGui frame = new VerkaeuferGui(verkaeufer);
	 ...
	 * Create the frame.
	 */
	public VerkaeuferGui(Entities.Verkaeufer verkaeufer) throws SQLException {
		...
		for(int i = 0; i<verkaeufer.getProduktList().size(); i++) {
			produktData[0] = verkaeufer.getProduktList().get(i).getProduktId();
			produktData[1] = verkaeufer.getProduktList().get(i).getProduktTitel();
			produktData[2] = verkaeufer.getProduktList().get(i).getProduktName();
			produktData[3] = verkaeufer.getProduktList().get(i).getPreis();
			produktData[4] = verkaeufer.getProduktList().get(i).getGarantiezeit();
			produktData[5] = verkaeufer.getProduktList().get(i).getFiliale();
			produktData[6] = verkaeufer.getProduktList().get(i).getFarbe();
		...
		for(int i = 0; i<verkaeufer.getKundeList().size(); i++) {
			kundeData[0] = verkaeufer.getKundeList().get(i).getId();
			kundeData[1] = verkaeufer.getKundeList().get(i).getName();
			kundeData[2] = verkaeufer.getKundeList().get(i).getNachname();
			kundeData[3] = verkaeufer.getKundeList().get(i).getFiliale();
			kundeData[4] = verkaeufer.getKundeList().get(i).getGeburtsdatum();
			kundeData[5] = verkaeufer.getKundeList().get(i).getGeschlecht();
			kundeData[6] = verkaeufer.getKundeList().get(i).getAdress();
			kundeData[7] = verkaeufer.getKundeList().get(i).getTelNr();
		...
		for(int i = 0; i<verkaeufer.getAufzeichnungList().size(); i++) {
			aufzeichnungData[0] = verkaeufer.getAufzeichnungList().get(i).getId();
			aufzeichnungData[1] = verkaeufer.getAufzeichnungList().get(i).getKunde();
			aufzeichnungData[2] = verkaeufer.getAufzeichnungList().get(i).getPersonal();
			aufzeichnungData[3] = verkaeufer.getAufzeichnungList().get(i).getProdukt();
			aufzeichnungData[4] = verkaeufer.getAufzeichnungList().get(i).getPreis();
			aufzeichnungData[5] = verkaeufer.getAufzeichnungList().get(i).getDatum();
			aufzeichnungData[6] = verkaeufer.getAufzeichnungList().get(i).getGarantiezeit();
			aufzeichnungData[7] = verkaeufer.getAufzeichnungList().get(i).getInfos();
		...
		btnQuit.setBounds(1128, 11, 100, 30);
		...
		kundeTable.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				try {
					textFieldKundeDelete.setText(kundeTable.getValueAt(kundeTable.getSelectedRow(), 0).toString());
				}catch(Exception ex){	
				}
			}
		...
		kundeVerwaltung.add(panel_1_1_1);
		...
		textKundeSum.setBounds(73, 319, 107, 14);
		...
		lblNewLabel_11_1_1.setBounds(10, 388, 90, 14);
		panel_1_1_1.add(lblNewLabel_11_1_1);
		
		textFieldKundeDelete = new JTextField();
		textFieldKundeDelete.setColumns(10);
		textFieldKundeDelete.setBounds(10, 402, 237, 23);
		panel_1_1_1.add(textFieldKundeDelete);
		
		JButton btnKundeDelete = new JButton("Kunde L\u00F6schen");
		btnKundeDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(textFieldKundeDelete.getText().length() != 0) {
					if(verkaeufer.deleteKunde(textFieldKundeDelete.getText())) {
						JOptionPane.showMessageDialog(null, "Kunde wird erfolgreich gelˆscht.");
						textFieldKundeDelete.setText(null);
						updateKundeModel();
					}
				} else {
		...
		btnKundeDelete.setBounds(10, 436, 237, 30);
		...
		btnKundeAddieren.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				KundeAddGui pag = new KundeAddGui(verkaeufer);
				pag.setVisible(true);
		...
		btnKundeAddieren.setBounds(10, 537, 237, 30);
		...
		btnAufzeichnungAddieren.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				VerkaufAddGui pag = new VerkaufAddGui(verkaeufer);
				pag.setVisible(true);
				pag.addWindowListener(new WindowAdapter() {
					@Override
					public void windowClosed(WindowEvent e) {
		...
		btnAufzeichnungLschen.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(textFieldAufzeichnungIdDelete.getText().length() != 0) {
					if(verkaeufer.deleteAufzeichnung(textFieldAufzeichnungIdDelete.getText(), bufferProdukt)) {
		...
		if(verkaeufer.getGeschlecht().equals("M")) {
			JLabel lblNewLabel = new JLabel("Willkommen "+ "Herr " + verkaeufer.getNachname());
			lblNewLabel.setFont(new Font("Tahoma", Font.PLAIN, 12));
			lblNewLabel.setBounds(0, 0, 880, 14);
			contentPane.add(lblNewLabel);
		} else if (verkaeufer.getGeschlecht().equals("F")) {
			JLabel lblNewLabel = new JLabel("Willkommen "+ "Frau " + verkaeufer.getNachname());
			lblNewLabel.setFont(new Font("Tahoma", Font.PLAIN, 12));
			lblNewLabel.setBounds(0, 0, 880, 14);
			contentPane.add(lblNewLabel);
		} else {
			JLabel lblNewLabel = new JLabel("Willkommen "+ "Sehr Geehrte " + verkaeufer.getNachname());
			lblNewLabel.setFont(new Font("Tahoma", Font.PLAIN, 12));
			lblNewLabel.setBounds(0, 0, 880, 14);
			contentPane.add(lblNewLabel);
		}
	...
	public static void sucheProduktModel(String id, String titel,String name, String minPreis, String maxPreis, String filiale, String farbe, char pt) throws SQLException {
		...
		clearModel.setRowCount(0);
		produktSum = 0;
		for(int i = 0; i<verkaeufer.produktSuche(id, titel, name, minPreis, maxPreis, filiale, farbe, pt).size(); i++) {
			produktData[0] = verkaeufer.produktSuche(id, titel, name, minPreis, maxPreis, filiale, farbe, pt).get(i).getProduktId();
			produktData[1] = verkaeufer.produktSuche(id, titel, name, minPreis, maxPreis, filiale, farbe, pt).get(i).getProduktTitel();
			produktData[2] = verkaeufer.produktSuche(id, titel, name, minPreis, maxPreis, filiale, farbe, pt).get(i).getProduktName();
			produktData[3] = verkaeufer.produktSuche(id, titel, name, minPreis, maxPreis, filiale, farbe, pt).get(i).getPreis();
			produktData[4] = verkaeufer.produktSuche(id, titel, name, minPreis, maxPreis, filiale, farbe, pt).get(i).getGarantiezeit();
			produktData[5] = verkaeufer.produktSuche(id, titel, name, minPreis, maxPreis, filiale, farbe, pt).get(i).getFiliale();
			produktData[6] = verkaeufer.produktSuche(id, titel, name, minPreis, maxPreis, filiale, farbe, pt).get(i).getFarbe();
			produktModel.addRow(produktData);
			produktSum++;
	...
	public static void sucheKundeModel(String id,String name, String nachname, String filiale) throws SQLException {
		...
		clearModel.setRowCount(0);
		kundeSum = 0;
		for(int i = 0; i<verkaeufer.kundeSuche(id, name, nachname, filiale).size(); i++) {
			kundeData[0] = verkaeufer.kundeSuche(id, name, nachname, filiale).get(i).getId();
			kundeData[1] = verkaeufer.kundeSuche(id, name, nachname, filiale).get(i).getName();
			kundeData[2] = verkaeufer.kundeSuche(id, name, nachname, filiale).get(i).getNachname();
			kundeData[3] = verkaeufer.kundeSuche(id, name, nachname, filiale).get(i).getFiliale();
			kundeData[4] = verkaeufer.kundeSuche(id, name, nachname, filiale).get(i).getGeburtsdatum();
			kundeData[5] = verkaeufer.kundeSuche(id, name, nachname, filiale).get(i).getGeschlecht();
			kundeData[6] = verkaeufer.kundeSuche(id, name, nachname, filiale).get(i).getAdress();
			kundeData[7] = verkaeufer.kundeSuche(id, name, nachname, filiale).get(i).getTelNr();
			kundeModel.addRow(kundeData);
			kundeSum++;
	...
	public static void sucheAufzeichnungModel(String id, String kunde,String personal, String produkt, char pt) throws SQLException {
		...
		clearModel.setRowCount(0);
		aufzeichnungSum = 0;
		for(int i = 0; i<verkaeufer.aufzeichnungSuche(id, kunde, personal, produkt, pt).size(); i++) {
			aufzeichnungData[0] = verkaeufer.aufzeichnungSuche(id, kunde, personal, produkt, pt).get(i).getId();
			aufzeichnungData[1] = verkaeufer.aufzeichnungSuche(id, kunde, personal, produkt, pt).get(i).getKunde();
			aufzeichnungData[2] = verkaeufer.aufzeichnungSuche(id, kunde, personal, produkt, pt).get(i).getPersonal();
			aufzeichnungData[3] = verkaeufer.aufzeichnungSuche(id, kunde, personal, produkt, pt).get(i).getProdukt();
			aufzeichnungData[4] = verkaeufer.aufzeichnungSuche(id, kunde, personal, produkt, pt).get(i).getPreis();
			aufzeichnungData[5] = verkaeufer.aufzeichnungSuche(id, kunde, personal, produkt, pt).get(i).getDatum();
			aufzeichnungData[6] = verkaeufer.aufzeichnungSuche(id, kunde, personal, produkt, pt).get(i).getGarantiezeit();
			aufzeichnungData[7] = verkaeufer.aufzeichnungSuche(id, kunde, personal, produkt, pt).get(i).getInfos();
			aufzeichnungModel.addRow(aufzeichnungData);
			aufzeichnungSum++;
